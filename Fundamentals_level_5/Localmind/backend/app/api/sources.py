...
@api_bp.route('/projects/<project_id>/ai-images/<filename>', methods=['GET'])
def get_ai_image(project_id: str, filename: str):
    """
    Serve an AI-generated image from the ai_outputs/images folder.

    Educational Note: This endpoint serves images generated by AI agents
    like the CSV analyzer agent. Images are rendered in chat using
    the [[image:FILENAME]] syntax which the frontend parses and converts
    to <img> tags pointing to this endpoint.

    Args:
        project_id: The project UUID
        filename: The image filename (e.g., source123_order_status.png)
